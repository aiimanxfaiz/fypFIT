...
def challenge():
    ...
    if user == "Student":
        ...
        if len(cursor) == 0:
            ...
            tk.Label(
                nw,
                text=f'{cursor[0][2]}\tSection: {cursor[0][1]}\tStudent ID: {cursor[0][3]}',
                font=('Consolas', 12, 'italic'),
                bg="#a0caf0"
            ).pack()

            # Maximize the window before calling mainloop
            nw.state('zoomed')

            m.destroy()
            timetable_stud.student_tt_frame_v2(nw, cursor[0][1])
            nw.mainloop()
        ...
        if len(cursor) == 0:
            ...
            nw.state('zoomed')  # Move this line inside the main loop
            nw.mainloop()
